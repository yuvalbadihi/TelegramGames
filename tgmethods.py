...
bot_id = 123456778 #ADD YOUR BOT ID TO HERE ! (keep it int)
...
class BotHandler:
    ...
    def send_poll(self, chat_id, question , options,startgametime):
        params = {'chat_id': chat_id, 'question' : question , 'options': options , 'is_anonymous' : False , 'open_period' : startgametime, 'parse_mode': 'HTML'}
        method = 'sendPoll'
        resp = requests.post(self.api_url + method, params)
        return resp

    def send_poll_quiz(self, chat_id, question , options,startgametime):
        params = {'chat_id': chat_id, 'question' : question , 'options': options,'type' : 'quiz','correct_option_id' : 0 , 'is_anonymous' : False , 'open_period' : startgametime, 'parse_mode': 'HTML'}
        method = 'sendPoll'
        resp = requests.post(self.api_url + method, params)
        return resp
    ...
    def open_keyboard(self, chat_id, text, keyboard):
        ...
        params = {'chat_id': chat_id, 'text': text,'reply_markup': keyboard ,'resize_keyboard' : True ,  'parse_mode': 'HTML'}
        method = 'sendMessage'
        resp = requests.post(self.api_url + method, params)
        return resp.json()
    ...
    def force_reply(self,chat_id,text):
    ...
    def restrict(self, chat_id,status):
        perm = {'can_send_messages' : status ,'can_send_media_messages' : status , 'can_send_polls' : False , 'can_send_other_messages' : status}
    ...
    def edit_title(self, chat_id):
        params = {'chat_id': chat_id,'user_id' : bot_id, 'custom_title': 'NONE', 'parse_mode': 'HTML'}
    ...
    def send_inline_callback_button(self, chat_id,message_text,button):
        button = json.dumps(button)
        params = {'chat_id': chat_id, 'text': message_text,'reply_markup': button,'cache_time':2 ,  'parse_mode': 'HTML'}
        method = 'sendMessage'
        resp = requests.post(self.api_url + method, params)
        return resp.json()
    ...
    def awnser_call_back_alert(self, call_back_id,text):
        ...
        params = {'callback_query_id': call_back_id,'text':text, 'show_alert':True, 'cache_time':5, 'next_offset':'null', 'parse_mode': 'HTML'}
    ...
    def awnser_call_back_alert_black(self, call_back_id,text):
        ...
        params = {'callback_query_id': call_back_id,'text':text, 'show_alert':False, 'cache_time':3, 'next_offset':'null', 'parse_mode': 'HTML'}
        method = 'answerCallbackQuery'
        resp = requests.post(self.api_url + method, params)
        return resp.json() 
